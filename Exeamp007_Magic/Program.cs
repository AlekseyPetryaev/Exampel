...
int xa = 10, ya = 1, 
    xb = 1,  yb = 30, 
    xc = 80, yc = 30;
Console.SetCursorPosition(xa, ya); //задаем_Переменные
...
while(count < 10) // указали сколько раз будет повтаряться

{
    ...
    {
        x = (x + xd) / 2; //тогда для Х это выражение 
        y = (y + yd) / 2; //тогда для У это выражение 
    }
    ...
    Console.WriteLine("+"); // задаем символ
    сount = count +1; // or /count +=1 ; or /cuont++; если у нас будет постоянное увеличение на один раз
}
*/
/*
int i=0;
